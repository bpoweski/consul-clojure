...
(comment (require '[clojure.tools.trace :as t]))

(defn base64->str [^String s]
  (DatatypeConverter/parseBase64Binary (.getBytes s)))
...
(defn consul
  ...
  (-> (consul-request conn method endpoint request)
...
(defn kv-get
  ...
  :keys      - Returns keys under the given prefix.
  :separator - List keys only up to a given separator.
  :raw?      - If true and used with a non-recursive GET, the response is just the raw value of the key, without any encoding."
  [conn k & {:as params :keys [recurse? raw?] :or {recurse? false raw? false}}]
  {:pre [(if recurse? (not raw?) true)]}
  (let [query-params                (cond-> (dissoc params :recurse? :raw?)
                                      recurse? (assoc :recurse "")
                                      raw?     (assoc :raw ""))
        {:keys [body] :as response} (consul conn :GET [:kv k] :query-params query-params)]
    body))
